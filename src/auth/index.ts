...
import { createUtilFunctions } from './utils/LocalStorageUtil';
...
const createTokenProvider = <T>({
    ...
    const setToken = (token: T | null) => {
        const tokenStatus: any = token;
        if (token && tokenStatus.status) {
            console.log(`Status message: ${tokenStatus.message}`);
            return;
        }
        if (token) {
